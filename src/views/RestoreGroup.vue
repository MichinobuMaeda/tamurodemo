<template>
  <div>
    <ButtonSecondary
      ...
      :label="$t('Restore item', { name: group.name })"
    ...
    >
      <v-card>
        ...
        <v-card-title class="headline dialogTitle">
          <v-icon>{{ icon('Undo delete') }}</v-icon>
          {{ $t('Restore item', { name: group.name }) }}
          <v-spacer />
          <v-icon
            color="gray"
        ...
        <v-card-text>
          <v-alert type="warning" text dense class="mt-2">
            {{ $t('Confirm restore', { name: group.name }) }}
          </v-alert>
        </v-card-text>
